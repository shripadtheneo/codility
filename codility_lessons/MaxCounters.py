...
def solution(N, A):
    ...
    for number in A:
        if number < (N + 1):
            if output_list[number - 1] < maximum_value:
                output_list[number - 1] = maximum_value
            output_list[number - 1] += 1
            if current_max < output_list[number - 1]:
                current_max = output_list[number - 1]
        else:
            maximum_value = current_max

    for index in range(N):
        if output_list[index] < maximum_value:
            output_list[index] = maximum_value
...
if __name__ == '__main__':
    ...
    A = raw_input()
    A = map(int, A.split())
    print "Please enter the lenght of output list: "
    N = int(raw_input())
    print solution(N, A)
