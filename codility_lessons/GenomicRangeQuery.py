...
def solution(S, P, Q):
    ...
    for i in range(len(P)):
        Ain = sumA[Q[i] + 1] - sumA[P[i]]
        Cin = sumC[Q[i] + 1] - sumC[P[i]]
        Gin = sumG[Q[i] + 1] - sumG[P[i]]
        Tin = sumT[Q[i] + 1] - sumT[P[i]]
        if Ain > 0:
            impact[i] = 1
        elif Cin > 0:
            impact[i] = 2
...
if __name__ == '__main__':
    ...
    S = raw_input()
    print "Please enter the P array: "
    P = raw_input()
    P = map(int, P.split())
    print "Please enter the Q array: "
    Q = raw_input()
    Q = map(int, Q.split())
